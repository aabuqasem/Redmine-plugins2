  ...
  module ApplicationHelperPatch
    ...
    module InstanceMethods
      ...
      def time_tag_with_usability(date)
        ...
        date = date.first
        if @project
          link_to(format_time(date), project_activity_path(@project, from: User.current.time_to_date(date)), title: format_time(date))
        else
      ...
      def render_project_jump_box_with_usability
        if !(Setting.plugin_usability || {})['dont_render_project_jump_box']
          render_project_jump_box_without_usability
        end
      end
